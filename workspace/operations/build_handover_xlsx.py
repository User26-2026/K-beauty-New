...
SCHEDULE = [
    ...
    (1, "ОСТАТКИ: начать пересчет с самых дорогих позиций", "Москва", "критично"),

    (2, "ОСТАТКИ: пересчет дорогих позиций, продолжение", "Москва", "критично"),
    ...
    (3, "ОСТАТКИ: закрыть 79 позиций, дающих 80% стоимости", "Москва", "критично"),
    ...
    (4, "ОСТАТКИ: пересчет следующих по стоимости позиций", "Москва", "критично"),
    ...
    (5, "ОСТАТКИ: зафиксировать факт по пересчитанному, отметить непосчитанное", "Москва", "критично"),
...
INSTRUCTIONS = [
    "Как дальше вести учет отгрузок: где отмечать, что продано и кому",
...
CHECKLIST = [
    ...
    ("Остатки", "Отмечено, какие позиции пересчитаны, а какие нет"),
    ("Остатки", "Акт сверки остатков подписан по пересчитанному"),
...
def sheet_schedule(wb):
    ...
    ws["A2"] = ("Последний рабочий день — 23 сентября. Главная задача — пересчитать "
                "склад по факту, начиная с самого дорогого. Вспоминать, что "
                "продано за прошлые годы, не нужно: фиксируем то, что есть сейчас.")
    ws["A2"].font = NOTE
