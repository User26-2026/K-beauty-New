...
HEAD = ['Товар', 'Схема', 'Цена', 'Себест.', 'Фулфилмент', 'База затрат',
        ...
        'Реклама (ДРР)', 'Соинвест, %', 'Соинвест ₽', 'Маржа до рекл.',
...
def build():
    ...
    input_cols = {'Цена', 'Себест.', 'Комиссия %', 'Логистика', 'Посл.миля',
                  'Выкуп %', 'Обр.лог база', 'Объём, л', 'Соинвест, %'}
    ...
    for k, d in enumerate(rows):
        ...
        vals = {
            ...
            'Соинвест, %': 0,  # подставь % из кабинета Ozon (Цены → Баллы за скидки)
        }
        # формулы
        formulas = {
            'Фулфилмент': f'={P["ff"]}',
        ...
        for name in HEAD:
            ...
            cell.border = border
            # форматы
            if name in ('Комиссия %', 'Выкуп %', 'Соинвест, %', 'Маржа % цены', 'ROI %'):
    ...
    widths = {'Товар': 42, 'Схема': 7, 'База затрат': 12, 'Обр.лог база': 12,
              'Реклама (ДРР)': 12, 'Соинвест, %': 11, 'Соинвест ₽': 11,
              'Маржа до рекл.': 13, 'Маржа % цены': 11, 'Цена безубыт.': 12}
    for name in HEAD:
    ...
    note = ws.cell(HROW + len(rows) + 3, 1,
                   ...
                   'выкуп, объём, соинвест %) и параметры B3:B8. Остальное — формулы. '
                   'Соинвест % возьми в кабинете (Цены → Баллы за скидки): баллы '
                   'компенсируют издержки, поэтому прибавляются к марже. '
