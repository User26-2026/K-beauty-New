...
HEAD = ['Товар', 'Схема', 'Цена', 'Себест.', 'Фулфилмент', 'База затрат',
        'Комиссия %', 'Комиссия ₽', 'Логистика', 'Посл.миля',
        'Обработка отправл.', 'Эквайринг', 'Налог', 'Выкуп %', 'Обр.лог база',
        'Обр.лог', 'Объём, л', 'Хранение',
        'Трафареты, %', 'Трафареты ₽', 'Продв. в поиске, %', 'Продв. в поиске ₽',
        'Реклама всего ₽', 'Соинвест, %', 'Соинвест ₽', 'Маржа до рекл.',
...
def build():
    ...
    for i, (label, val, fmt) in enumerate(params):
        ...
        cell.border = border
        cell.font = bold
    # ДРР теперь задаётся по строкам (Трафареты % / Продв. в поиске %), не глобально.
    P = {'tax': '$B$3', 'acq': '$B$4', 'ff': '$B$5', 'stl': '$B$6', 'std': '$B$7'}
    ...
    input_cols = {'Цена', 'Себест.', 'Комиссия %', 'Логистика', 'Посл.миля',
                  'Обработка отправл.', 'Выкуп %', 'Обр.лог база', 'Объём, л',
                  'Трафареты, %', 'Продв. в поиске, %', 'Соинвест, %'}
    ...
    for k, d in enumerate(rows):
        ...
        vals = {
            ...
            'Обработка отправл.': 0,  # ₽/шт, из тарифов кабинета (для FBS/rFBS)
            'Выкуп %': d['buyout'], 'Обр.лог база': d['retb'], 'Объём, л': d['vol'],
            'Трафареты, %': DRR,       # ДРР по трафаретам (CPM/CPC), доля от цены
            'Продв. в поиске, %': 0,   # ставка продвижения в поиске, % от продажи
        ...
        formulas = {
            ...
            # Реклама по тарифам Ozon: трафареты (доля от цены) + продвижение
            # в поиске (ставка % от продажи). Сумма — общий рекламный расход.
            'Трафареты ₽': f'={cr("Цена")}*{cr("Трафареты, %")}',
            'Продв. в поиске ₽': f'={cr("Цена")}*{cr("Продв. в поиске, %")}',
            'Реклама всего ₽': f'={cr("Трафареты ₽")}+{cr("Продв. в поиске ₽")}',
            ...
            'Маржа до рекл.': (f'={cr("Цена")}-{cr("Комиссия ₽")}-{cr("Логистика")}'
                              f'-{cr("Посл.миля")}-{cr("Обработка отправл.")}'
            ...
            'Цена безубыт.': (f'=({cr("Логистика")}+{cr("Посл.миля")}'
                             f'+{cr("Обработка отправл.")}+{cr("Обр.лог")}'
                             f'+{cr("Хранение")}+{cr("База затрат")})'
                             f'/(1-({cr("Комиссия %")}+{P["acq"]}+{P["tax"]}'
                             f'+{cr("Трафареты, %")}+{cr("Продв. в поиске, %")}'
                             f'-{cr("Соинвест, %")}))'),
        }
        ...
        for name in HEAD:
            ...
            cell.border = border
            # форматы
            if name in ('Комиссия %', 'Выкуп %', 'Трафареты, %', 'Продв. в поиске, %',
                        'Соинвест, %', 'Маржа % цены', 'ROI %'):
    ...
    widths = {'Товар': 42, 'Схема': 7, 'База затрат': 12, 'Обр.лог база': 12,
              'Обработка отправл.': 13, 'Трафареты, %': 11, 'Трафареты ₽': 11,
              'Продв. в поиске, %': 13, 'Продв. в поиске ₽': 13, 'Реклама всего ₽': 13,
    ...
    note = ws.cell(HROW + len(rows) + 3, 1,
                   ...
                   'обработка отправления, выкуп, объём, реклама %, соинвест %) '
                   'и параметры B3:B7. Остальное — формулы. Реклама разбита по '
                   'тарифам: Трафареты % (доля от цены) и Продвижение в поиске % '
                   '(ставка от продажи). Соинвест % возьми в кабинете (Цены → '
                   'Баллы за скидки): баллы компенсируют издержки и прибавляются '
                   'к марже. Меняешь цену в колонке C → маржа и ROI пересчитываются.')
