...
def build():
    ...
    for k, d in enumerate(rows):
        ...
        formulas = {
            ...
            # Наценка за нелокальность = % (B8) от цены.
            'Наценка нелок., ₽': f'={cr("Цена")}*{P["nonlocal"]}',
            ...
            # Наценка (% от цены) и налог ((1-СПП)*ставка) — в знаменателе как доли.
            'Цена безубыт.': (f'=({cr("Логистика")}+{cr("Доставка до ПВЗ")}'
                             f'+{cr("Обр.лог")}+{cr("Хранение")}+{cr("База затрат")})'
                             f'/(1-({cr("Комиссия %")}+{P["acq"]}+(1-{cr("СПП, %")})*{P["tax"]}'
                             f'+{P["nonlocal"]}+{cr("Оплата за клик, %")}+{cr("Вывод в топ, %")}'
                             f'-{cr("Соинвест, %")}))'),
        }
