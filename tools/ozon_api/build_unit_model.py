...
HEAD = ['Товар', 'Цена', 'Себест.', 'Фулфилмент', 'База затрат',
        'Комиссия %', 'Комиссия ₽', 'Логистика', 'Доставка до ПВЗ',
...
def build():
    ...
    input_cols = {'Цена', 'Себест.', 'Комиссия %', 'Логистика', 'Доставка до ПВЗ',
    ...
    for k, d in enumerate(rows):
        ...
        # входные значения
        vals = {
            'Товар': d['offer'], 'Цена': d['price'],
            'Себест.': round(d['cost'], 2), 'Комиссия %': d['comm'],
            'Логистика': d['logi'], 'Доставка до ПВЗ': d['lastm'],
        ...
        formulas = {
            ...
            'Маржа до рекл.': (f'={cr("Цена")}-{cr("Комиссия ₽")}-{cr("Логистика")}'
                              f'-{cr("Доставка до ПВЗ")}-{cr("Эквайринг")}-{cr("Налог")}'
            ...
            'Цена безубыт.': (f'=({cr("Логистика")}+{cr("Доставка до ПВЗ")}+{cr("Обр.лог")}'
        ...
        for name in HEAD:
            ...
            cell.border = border
            # форматы
            if name in ('Комиссия %', 'Выкуп %', 'Оплата за клик, %', 'Вывод в топ, %',
                        'ДРР %', 'Соинвест, %', 'Маржа % цены', 'ROI %'):
    ...
    # ширины
    widths = {'Товар': 42, 'База затрат': 12, 'Обр.лог база': 12,
    ...
    note = ws.cell(HROW + len(rows) + 3, 1,
                   'Только FBO. Жёлтые ячейки — вход (цена, себест., комиссия, '
                   'логистика, выкуп, объём, реклама %, соинвест %) и параметры '
                   'B3:B7. Остальное — формулы. Реклама по актуальным форматам '
                   'Ozon: Оплата за клик % (CPC, продвижение товаров — ДРР) и '
                   'Вывод в топ % (CPO — ставка от цены заказа). Соинвест % возьми '
                   'в кабинете (Цены → Баллы за скидки): баллы компенсируют '
                   'издержки и прибавляются к марже. Меняешь цену в колонке B → '
                   'маржа и ROI пересчитываются.')
