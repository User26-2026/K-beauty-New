...
# Итоговый список на опт: из того, что можем отгрузить, вычли позиции,
# которые выгоднее продать самим.
FINAL = ["№", "Наименование", "Количество, шт", "Цена, руб", "Сумма, руб",
         "Прибыль, руб", "Останется у нас, шт", "Нам хватит на, мес"]
FINAL_WIDTHS = [5, 66, 15, 12, 15, 14, 16, 15]

# Не отдавать: на WB та же партия дает больше и успевает разойтись.
HOLD = ["№", "Наименование", "Просит, шт", "Продажи в месяц, шт",
        "Распродадим сами за, мес", "Прибыль оптом, руб", "Прибыль на WB, руб",
        "Разница, руб", "Во сколько раз WB выгоднее"]
HOLD_WIDTHS = [5, 68, 12, 15, 16, 15, 15, 14, 16]
# Дольше этого срока держать позицию ради маржи WB нет смысла: деньги
# стоят, а маржа приходит по частям годами.
SELL_MONTHS = 12
...
def main(source, sales_path, container_path, keep, target, list_only=False,
    ...
    hold = loss[[want <= monthly * SELL_MONTHS for want, monthly in
                 zip(loss["Просит, шт"], loss["Продажи в месяц, шт"])]] \
        if len(loss) else loss
    if len(hold):
        hold = hold.assign(**{"Распродадим сами за, мес": [
            round(want / monthly, 1) for want, monthly in
            zip(hold["Просит, шт"], hold["Продажи в месяц, шт"])]})
        hold = hold.rename(columns={"Потеряем, руб": "Разница, руб"})
        hold = hold[HOLD]
        hold = hold.sort_values("Разница, руб", ascending=False)
        hold["№"] = range(1, len(hold) + 1)
        write(book, "НЕ ОТДАВАТЬ, ПРОДАДИМ САМИ", HOLD, hold.values.tolist(),
              HOLD_WIDTHS,
              ["", "ИТОГО", int(hold["Просит, шт"].sum()),
               int(hold["Продажи в месяц, шт"].sum()), "",
               int(hold["Прибыль оптом, руб"].sum()),
               int(hold["Прибыль на WB, руб"].sum()),
               int(hold["Разница, руб"].sum()), ""], "CDFGH", "E", name_at=1)
        print(f"Не отдавать: {len(hold)} позиций, "
              f"разница {int(hold['Разница, руб'].sum()):,} руб".replace(",", " "))
    if len(hold):
        # Итоговый список: убираем из предложения все, что решили оставить.
        держим = set(hold["Наименование"])
        final = inside[~inside["Наименование"].isin(держим)][
            ["№", "Наименование", "Можем отдать, шт", "Цена, руб", "Сумма, руб",
             "Прибыль, руб", "Останется у нас, шт", "Нам хватит на, мес"]]
        final.columns = FINAL
        final = final.sort_values("Сумма, руб", ascending=False)
        final["№"] = range(1, len(final) + 1)
        write(book, "ИТОГОВЫЙ СПИСОК НА ОПТ", FINAL, final.values.tolist(),
              FINAL_WIDTHS,
              ["", "ИТОГО", int(final["Количество, шт"].sum()), "",
               int(final["Сумма, руб"].sum()), int(final["Прибыль, руб"].sum()),
               int(final["Останется у нас, шт"].sum()), ""], "CEFG", "D",
              name_at=1)
        print(f"Итоговый список на опт: {len(final)} позиций, "
              f"{int(final['Количество, шт'].sum()):,} шт на "
              f"{int(final['Сумма, руб'].sum()):,} руб".replace(",", " "))
    if len(loss):
    ...
    if "ИТОГОВЫЙ СПИСОК НА ОПТ" in book.sheetnames:
        # Итоговый список — второй лист, сразу после свода по брендам.
        book.move_sheet("ИТОГОВЫЙ СПИСОК НА ОПТ", -(len(book.sheetnames) - 2))
